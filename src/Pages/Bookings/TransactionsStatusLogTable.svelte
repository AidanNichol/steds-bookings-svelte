<script>
  ...
  $: logit('Transaction Data Raw', data, $activityLogData);
...
<div class="scrollBox" />
<svg {width} {height} viewBox={`0 0 ${bodyW} ${bodyH}`}>
  <Icons />

  <rect x="0" y="0" width={bodyW} height={bodyH} class="area" />

  {#each entries as { day, expenditure, income, y, expSz, incSz, expHistoric, incHistoric }}
    {#if expSz > 0}
      <g class:historic={_.every(expenditure, 'historic')}>
        <DayHeader x={expLeft} {y} width={expWidth} height={expSz} title={day} />
        {#each expenditure as { walkId, status, name, venue, Allocations: allocs, memberId, owing, y, sz, historic, log2, maxX, refundId, req, amount }, i}
          <g
            class="expenditure"
            class:clickable={isClickable(walkId, status, refundId, req)}
            class:historic
            class:outStanding={owing}
          >
            {#if walkId && status}
              <text x={expLeft + hPad} y={y + sz / 2} {...LM}
                >{`${walkId.substr(1)} ${name} ${venue}`}</text
              >
              <rect
                x={expRight - maxX - 0.3}
                y={y + 0.1}
            ...
            {#if refundId && req}
              <text x={expLeft + hPad} y={y + sz / 2} {...LM}>{`Refund £${amount}`}</text>
              <Icon {req} x={expLeft + hPad + 6} y={y + sz / 2 - 0.49} />
            {/if}

            y += 0.5;
            {#each allocs as { id, amount, paymentId, historic }, j}
              <g class:historic class="allocs">
                <text x={expRight - 0.24} y={y + j + 0.5} {...RM} class:owing={!paymentId}
                  >{amount ? `£${amount}` : '—'}</text
                >
              </g>
            {/each}
    ...
    {#if incSz}
      <g class:historic={_.every(income, 'historic')}>
        <DayHeader x={incLeft} {y} width={incWidth} height={incSz} title={day} />

        {#each income as { Allocations: allocs, req, paymentId, historic, amount, y, sz, bankingId }}
          <g class="payment" class:clickable={!bankingId} class:historic>
            ...
            {#each allocs as { id, amount, bookingId, historic }, j}
              <g class:historic>
                <text x={incLeft + hPad} y={y + j + 0.5} class:avail={!bookingId} {...LM}
                  >{amount ? `£${amount}` : '—'}</text
                >
              </g>
            {/each}
